...
contract VotingDApp {
    ...
    modifier votingActive(){
        require(
            block.timestamp >= voting_start_time && block.timestamp <= voting_end_time,
    ...
    function createCandidate(string memory id,string memory name, string memory image) external onlyAdmin{
        
        require(bytes(candidates[id].id).length == 0, "Candidate ID already exists.");
    ...
    function vote(address voterID,string memory candidateID) external votingActive{
        // ensuring voter hasn't voted
        require(!voters[voterID].hasVoted,"You have already voted");
        // checking if the user existed
        require(bytes(candidates[candidateID].id).length != 0, "Candidate ID does not exist.");
    ...
    function Tallying ()external view votingClosed returns (Candidate [] memory){
    ...
    function getCandidateDetails(string memory candidateID) external view returns (Candidate memory){
        require(bytes(candidates[candidateID].id).length != 0, "Candidate doesnt exist");
